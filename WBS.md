...
| 18 | Phase 1 통합 테스트 및 버그 수정 | 완료 기준 항목 전체 체크 | ⬜ 예정 |
| 19 | Phase 1 배포 확인 및 태깅 (`v0.1.0`) | Vercel Production에서 데모 확인 | ⬜ 예정 |

## Phase 2 — 3D 뷰어 전환: Three.js 맛보기

| Day | 작업 내용 | 산출물 / 완료 기준 | 상태 |
| --- | --- | --- | --- |
| 20 | 3D Canvas 기본 세팅 + 바닥(`PlaneGeometry`) 구현 | 3D 페이지에 바닥 렌더링 | ⬜ 예정 |
| 21 | 로봇 큐브(`BoxGeometry`) + `robotStore` 연동 렌더링 | 3D 공간에 로봇 위치 반영 | ⬜ 예정 |
| 22 | `useFrame` 기반 실시간 좌표 업데이트 | 좌표 변경 시 큐브 이동 | ⬜ 예정 |
| 23 | OrbitControls 카메라 조작 추가 | 드래그로 360도 회전 | ⬜ 예정 |
| 24 | 2D/3D 전환 토글 버튼 UI | 버튼 클릭 시 화면 전환 | ⬜ 예정 |
| 25 | 토글 상태에 따른 컴포넌트 스위칭 로직 | 상태 공유하며 뷰만 교체 | ⬜ 예정 |
| 26 | 로봇 더블클릭 카메라 추적 로직 (기본) | 특정 로봇 따라가기 동작 | ⬜ 예정 |
| 27 | 카메라 추적 보간(lerp) 부드럽게 처리 | 카메라 이동이 자연스러움 | ⬜ 예정 |
| 28 | Phase 2 통합 테스트 및 버그 수정 | 완료 기준 항목 전체 체크 | ⬜ 예정 |
| 29 | Phase 2 배포 확인 및 태깅 (`v0.2.0`) | Vercel Production에서 3D 토글 데모 확인 | ⬜ 예정 |

## Phase 3 — 관제 이벤트 및 제어

| Day | 작업 내용 | 산출물 / 완료 기준 | 상태 |
| --- | --- | --- | --- |
| 30 | `FleetEvent` 타입 정의 + Mock 서버 이벤트 발행 로직 | 무작위 충돌 이벤트 발행 확인 | ⬜ 예정 |
| 31 | FE WebSocket 이벤트 구독 훅 구현 | 이벤트 수신 콘솔 확인 | ⬜ 예정 |
| 32 | 토스트 알림 UI 구현 | 이벤트 수신 시 토스트 표시 | ⬜ 예정 |
| 33 | 2D/3D 화면 경고 마커 표시 | 해당 로봇 위치에 ⚠️ 표시 | ⬜ 예정 |
| 34 | 지도 클릭 좌표 선택 UI | 빈 좌표 클릭 시 선택 상태 표시 | ⬜ 예정 |
| 35 | 원격 이동 명령 패널 UI | 선택 좌표 + 전송 버튼 구현 | ⬜ 예정 |
| 36 | FE → Mock 서버 명령 전송 (WS publish) | 명령 전송 시 서버 로그 확인 | ⬜ 예정 |
| 37 | Mock 서버: 명령 수신 후 로봇 이동 반영 | 로봇이 지정 좌표로 이동 시작 | ⬜ 예정 |
| 38 | Phase 3 통합 테스트 및 버그 수정 | 완료 기준 항목 전체 체크 | ⬜ 예정 |
| 39 | Phase 3 최종 배포 + 데모 정리 (`v1.0.0`) | Vercel Production 최종 데모, README 데모 링크 갱신 | ⬜ 예정 |
...
- Phase 완료 Day(19, 29, 39)에는 실제 배포 URL에서 눈으로 확인 후 `git tag`로 마일스톤을 남긴다.
